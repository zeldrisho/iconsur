...
Pipeline: `vp pack` (tsdown single-CJS bundle, all deps inlined — pkg cannot load external ESM like `plist@5`) → copy `src/mask.png` into `dist/` → `@yao-pkg/pkg` with `.pkgrc.json` assets. Targets are `node24` for macOS, Linux, and Windows × arm64 + x64; outputs land in `dist/iconsur-<platform>-<arch>[.exe]` (gitignored) and are what CI attaches to GitHub Releases. Only the macOS binaries have full functionality — see `README.md`.
...
`CHANGELOG.md` is generated by git-cliff and excluded from auto-formatting (`fmt.ignorePatterns` in `vite.config.ts`).

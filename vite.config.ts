...
export default defineConfig({
  ...
  // Note: no `*.md` rule — CHANGELOG.md is generated by git-cliff and must
  // never be rewritten by a hook (fmt.ignorePatterns above already excludes it
  // from `vp check`/`vp fmt`); markdown formatting is enforced by CI's `vp check`.
  staged: {
    "*.ts": "vp check --fix",
